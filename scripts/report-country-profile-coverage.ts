...
const { review: indicatorReview, ...indicators } = read("src/data/country-indicators.json");
indicatorCandidateSchema.parse(indicators);
const indicatorRaw = readFileSync("research/country-profiles/top20-indicators-2026-09-17.candidate.json", "utf8");
const approvedIndicatorReview = countryProfileReviewSchema.parse(read("research/country-profiles/top20-indicators-2026-09-17.review.json"));
...
const rows = cohort.passports.map((passport) => {
...
const result = {
  scope: "Frozen live ranks 1–20 including every tie; bundled approved content, not production deployment verification.",
  ...
  unresolvedLegalFacts: topics.filter((topic) => cohort.passports.some((entry) => entry.code === topic.code)).flatMap((topic) => topic.facts.filter((fact) => fact.state === "not_established").map((fact) => `${topic.code}/${topic.topic}/${fact.id}`)),
...
if (process.argv.includes("--require-complete") && (result.profilesWithAllRequiredOutcomes !== 47 || !result.retainedIndiaPilot)) process.exitCode = 1;
