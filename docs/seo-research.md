...
A second, deliberately small US/English alias check after the account refill tested 48 common demonym and abbreviation variants for USD 0.09. Germany–United States and India–United States returned approximately 20 monthly searches; Australia–New Zealand, Australia–United Kingdom, Ireland–Canada, and Italy–United States returned approximately 10. Those six pairs are now canonical pages in the core sitemap too, taking the reviewed set to 22. Other non-zero variants confirmed already-selected pairs or merely reversed their word order, so they did not create duplicate URLs. This targeted extension is preferred to another combinatorial sweep.

## Agent and Search Console demand update — 7 September 2026

Cloudflare's supplied agent exports show that passport–destination evidence, not generic editorial content, is the main machine-consumption surface. Relationship URLs account for 5,799 of 7,884 requests (73.6%) in the 5,000-row popular-pages export. Markdown accounts for 929 requests (11.8%), including 918 relationship-page requests. The shared Open Graph image accounts for 1,753 requests; it remains one small cacheable 1.6 KB asset rather than per-page generated media.

The 382-request demand-signals export contains 298 recognizable relationship requests and nine passport or destination directory requests. Another 75 requests are non-content probes for environment files, keys, framework internals, or administrative endpoints and must remain 404 responses. The legitimate miss is an identity-stability regression: the live upstream renamed ISO `NR` from the common English “Nauru” to the Nauruan endonym “Naoero.” That generated 89 requests to temporary `naoero` URLs while 22 requests using established `nauru` URLs were not resolved. ISO-backed display-name normalization now keeps `nauru` canonical and permanently redirects either spelling.
...
## Content-based search eligibility — 16 September 2026

The latest Search Console validation export contained 1,327 unique URLs: 1,000 pending and 327 failed validations, with crawls through 15 September. Before this change, comparison against the live evidence matrix and all eight sitemap shards classified 128 as now indexable, 61 as redirects to indexable pages, 900 as non-exact relationship pages (including five redirects), and 238 as intentionally excluded tool states. Thirty sampled HTML responses agreed with those rules; there was no sitemap/evidence mismatch in the export. This established implementation consistency, not that every exclusion was desirable.

The owner approved replacing the exact-verification-only search rule with a content-quality rule. Current reviewed conditional explanations need substantive conditions, possible routes, and complete official citations. Sourced corrections can qualify within their recorded review window. Bare imported labels and empty placeholders still do not qualify. Non-exact pages now use neutral visa-requirements titles, visibly qualify the ranking label, and provide inline conditional citations in HTML and Markdown. No new legal claims, evidence promotions, score changes, or verification upgrades are part of this change.

On the bundled snapshot, the change adds 1,142 eligible relationship pages (1,102 conditional and 40 reviewed corrections) while exact coverage stays 40,941 / 44,974. HTML, native Markdown, negotiated Markdown, and sitemap eligibility share the decision. Sitemap tests validate the decision against the whole matrix. The total bundled sitemap inventory becomes 42,567 including six new comparison pages; live totals can differ with the upstream snapshot and must be measured after publication.

The six additional comparisons are Russia–US, US–UAE, Brazil–US, Germany–UK, India–Singapore, and South Africa–Morocco. Five appear as crawled query URLs in the export; this is evidence of discovery, not keyword volume or human search demand. Germany–UK is a deliberate European visitor-access benchmark, explicitly distinguished from residence/work rights. Each page has live scores, directional destination differences, context, and HTML/Markdown routes. Both query orders and registered reversed-name aliases redirect to one canonical URL. US–UAE is explicitly a mobility benchmark, not an ordinary second-citizenship recommendation. No paid keyword API calls were needed, and arbitrary complex tool states remain excluded.

The release should be evaluated using the submitted-pages filter and live URL inspection, not by trying to make “All known pages / Excluded by noindex” reach zero. Google requires indexed, snippet-eligible pages for AI Overview/AI Mode supporting links; `noindex` is not a request-cost optimization. Relevant primary references: [content quality](https://developers.google.com/search/docs/fundamentals/creating-helpful-content), [canonicalization](https://developers.google.com/search/docs/crawling-indexing/consolidate-duplicate-urls), [AI Search eligibility](https://developers.google.com/search/docs/appearance/ai-features).
