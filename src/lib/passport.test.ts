...
describe("passport calculations", () => {
  ...
  it("renders directly disproved imported categories as pending unknowns", () => {
    ...
    for (const [code, destinationCode, importedStatus, mobilityScore] of [
      ["GE", "DO", "visa_required", 0],
      ["PG", "DO", "visa_free", 1],
      ["SS", "DO", "visa_required", 0],
      ["UY", "GY", "visa_free", 1],
      ["ID", "GY", "visa_free", 1],
      ["CU", "GY", "visa_free", 1],
    ] as const) {
    ...
    for (const code of ["UA", "RS"] as const) {
      expect(applyAccessOverrides({
        ...
        statuses: { GF: "unknown", PF: "unknown", YT: "unknown", NC: "unknown", RE: "unknown" },
        mobilityScore: 0,
      });
    }
    ...
    for (const code of ["BH", "MC", "RW"] as const) {
      expect(applyAccessOverrides({
        code,
        name: code,
        statuses: { [code]: "citizenship", VU: "visa_free" },
        mobilityScore: 1,
      })).toMatchObject({ statuses: { [code]: "citizenship", VU: "unknown" }, mobilityScore: 0 });
    }

    for (const [code, destinationCode] of [["NR", "BZ"], ["AD", "GT"]] as const) {
      expect(applyAccessOverrides({
        code,
        name: code,
        statuses: { [code]: "citizenship", [destinationCode]: "visa_free" },
        mobilityScore: 1,
      })).toMatchObject({
    ...
    for (const [code, importedStatus] of [
      ["UA", "visa_free"],
      ["RS", "visa_free"],
      ["EC", "visa_required"],
      ["VU", "visa_required"],
      ...
      expect(applyAccessOverrides({
        code,
        name: code,
        statuses: { [code]: "citizenship", FW: importedStatus },
  ...
  it("applies the ordinary U.S. passport restriction for North Korea", () => {
    expect(applyVerifiedAccessOverrides({
      code: "US",
      name: "United States",
      statuses: { US: "citizenship", KP: "visa_required" },
      mobilityScore: 0,
    })).toMatchObject({
      statuses: { US: "citizenship", KP: "entry_restricted" },
      mobilityScore: 0,
    });
  });
