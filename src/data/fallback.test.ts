...
describe("bundled passport snapshot", () => {
  ...
  it("applies verified destination-authority corrections to the bundled snapshot", () => {
    ...
    expect(snapshot.passports.ZA.statuses.KG).toBe("evisa");
  ...
  it("bundles reviewed unknowns without inventing replacement access categories", () => {
    ...
    expect(snapshot.passports.TR.statuses.LY).toBe("unknown");
    expect(snapshot.passports.TR.mobilityScore).toBe(107);
    ...
    expect(snapshot.passports.CA.statuses.TM).toBe("visa_on_arrival");
    expect(snapshot.passports.CA.mobilityScore).toBe(185);
    expect(snapshot.passports.PL.statuses.LR).toBe("visa_required");
    expect(snapshot.passports.PL.mobilityScore).toBe(183);
    expect(snapshot.passports.US.statuses.BF).toBe("unknown");
    ...
    expect(snapshot.manifest.passports.find(({ code }) => code === "SS")?.rank).toBe(100);
    expect(snapshot.passports.CU.statuses.MN).toBe("visa_required");
    expect(snapshot.passports.CU.mobilityScore).toBe(56);
    expect(snapshot.manifest.passports.find(({ code }) => code === "CU")?.rank).toBe(86);
    expect(snapshot.passports.IN.statuses.HN).toBe("visa_free");
    expect(snapshot.passports.IN.mobilityScore).toBe(58);
    expect(snapshot.manifest.passports.find(({ code }) => code === "IN")?.rank).toBe(84);
  });
